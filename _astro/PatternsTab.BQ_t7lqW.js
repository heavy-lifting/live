import{j as e}from"./jsx-runtime.DDS51bD4.js";import{u as C,$ as v,c as w,d as f,g as D,e as k,f as B,p as x,h as G,s as E,j as p,k as r,m as j,n as S}from"./useClient.C3xvyNXE.js";import{r as u}from"./index.DIn8BZQ7.js";function N(...s){return s.filter(Boolean).join(" ")}function _({value:s,onChange:t,items:a}){return e.jsx("div",{className:"flex max-w-lg",children:Object.entries(a).map(([n,c],i,h)=>e.jsx("button",{onClick:()=>t(n),className:N("px-2 border-b h-8 whitespace-nowrap",s===n?"border-foreground":"border-transparent"),children:c.toLowerCase()},n))})}const R=`// Koji Kondo - Swimming (Super Mario World)
stack(
  seq(
    "~",
    "~",
...
.pianoroll({fold:1})`,P=`// Koji Kondo - Princess Zelda's Rescue
stack(
  // melody
...
.pianoroll({fold:1})
`,q=`// adapted from a Barry Harris excercise
"0,2,[7 6]"
  .add("<0 1 2 3 4 5 7 8>")
  .scale('C bebop major')
  .transpose("<0 1 2 1>/8")
...
//.pianoroll({fold:1})`,K=`// "Wavy kalimba"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
.clip(.5)
.piano()
.pianoroll()`,z=`// Hirokazu Tanaka - World 1-1
stack(
  // melody
  note(\`<
...
note(
  ...
  .gain(.5)
  .room(.5)
  .lpa(.125).lpenv(-2).v("8:.125").fanchor(.25)`,U=`// "Festival of fingers 3"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
n("[-7*3],0,2,6,[8 7]")
  ...
  .slow(2).piano()
//.pianoroll({maxMidi:160})`,V=`// "Melting submarine"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
stack(
    ...
    .delaytime(".16 | .33")
    .delayfeedback(".6 | .8")
  ).sometimes(x=>x.speed("-1"))`,ee=`// "Orbit"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
stack(
    ...
    .delayfeedback(.7)
    .orbit(2)
  ).sometimes(x=>x.speed("-1"))`,se=`// "Belldub"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
).reset("<x@7 x(5,8,-1)>")`,ce=`// "Jux und tollerei"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
.note()
//.pianoroll()
.csound('CoolSynth')`,re=`// "Lounge sponge"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
...
`,be=Object.freeze(Object.defineProperty({__proto__:null,amensister:le,arpoon:de,barryHarris:q,bassFuge:Z,belldub:se,blippyRhodes:O,caverave:M,chop:Q,csoundDemo:ie,delay:X,dinofunk:ae,echoPiano:$,festivalOfFingers:L,festivalOfFingers3:U,flatrave:ne,giantSteps:T,goodTimes:W,holyflute:te,juxUndTollerei:ce,loungeSponge:re,meltingsubmarine:V,orbit:ee,randomBells:J,sampleDemo:oe,sampleDrums:Y,sml1:z,swimming:R,undergroundPlumber:H,waa2:I,wavyKalimba:K,zeldasRescue:P},Symbol.toStringTag,{value:"Module"}));Object.fromEntries(Object.entries(be).map(([s,t],a)=>[a,{id:a,code:t,collection:"Stock Examples"}]));const me=()=>{const s=C(v),t=C(w),a=u.useMemo(()=>{const c=new Map;return c.set(f.featured,s),c.set(f.public,t),c},[s,t]);return{patterns:u.useMemo(()=>Object.assign({},...a.values()),[a]),collections:a}};function pe(...s){return s.filter(Boolean).join(" ")}function he({pattern:s}){const t=u.useMemo(()=>D(s.code),[s]);let a=t.title;if(a==null){const n=new Date(s.created_at);isNaN(n)||(a=n.toLocaleDateString())}return a==null&&(a=s.hash),a==null&&(a="unnamed"),e.jsx(e.Fragment,{children:`${s.id}: ${a} by ${Array.isArray(t.by)?t.by.join(","):"Anonymous"}`})}function ue({showOutline:s,onClick:t,pattern:a,showHiglight:n}){return e.jsx("a",{className:pe("mr-4 hover:opacity-50 cursor-pointer block",s&&"outline outline-1",n&&"bg-selection"),onClick:t,children:e.jsx(he,{pattern:a})})}function y({patterns:s,activePattern:t,onClick:a,started:n}){const c=B(),h=x(c).id;return e.jsx("div",{className:"font-mono text-sm",children:Object.values(s).reverse().map(d=>{const l=d.id;return e.jsx(ue,{pattern:d,showHiglight:l===h,showOutline:l===t&&n,onClick:()=>a(l)},l)})})}function b({children:s,onClick:t,label:a,labelIsHidden:n}){return e.jsxs("button",{className:"hover:opacity-50",onClick:t,title:a,children:[n!==!0&&a,s]})}function fe({context:s}){const t=k(),a=B(),n=x(a),{userPatterns:c,patternFilter:i}=G(),d=me().collections,l=(o,m=!1)=>{s.handleUpdate(o,m)},F=n?.id,g=!window.parent?.location.pathname.includes("oodles");return e.jsxs("div",{className:"px-4 w-full dark:text-white text-stone-900 space-y-2 pb-4 flex flex-col overflow-hidden max-h-full",children:[e.jsx(_,{value:i,onChange:o=>E.setKey("patternFilter",o),items:p}),i===p.user&&e.jsx("div",{children:e.jsxs("div",{className:"pr-4 space-x-4 border-b border-foreground flex max-w-full overflow-x-auto",children:[e.jsx(b,{label:"new",onClick:()=>{const{data:o}=r.createAndAddToDB();l(o)}}),e.jsx(b,{label:"duplicate",onClick:()=>{const{data:o}=r.duplicate(n);l(o)}}),e.jsx(b,{label:"delete",onClick:()=>{const{data:o}=r.delete(F);l({...o,collection:r.collection})}}),e.jsxs("label",{className:"hover:opacity-50 cursor-pointer",children:[e.jsx("input",{style:{display:"none"},type:"file",multiple:!0,accept:"text/plain,application/json",onChange:o=>j(o.target.files)}),"import"]}),e.jsx(b,{label:"export",onClick:S}),e.jsx(b,{label:"delete-all",onClick:()=>{const{data:o}=r.clearAll();l(o)}})]})}),e.jsxs("section",{className:"flex overflow-y-scroll max-h-full flex-col",children:[i===p.user&&e.jsx(y,{onClick:o=>l({...c[o],collection:r.collection},g),patterns:c,started:s.started,activePattern:t,viewingPatternID:F}),i!==p.user&&Array.from(d.keys()).map(o=>{const m=d.get(o);return e.jsxs("section",{className:"py-2",children:[e.jsx("h2",{className:"text-xl mb-2",children:o}),e.jsx("div",{className:"font-mono text-sm",children:e.jsx(y,{onClick:A=>l({...m[A],collection:o},g),started:s.started,patterns:m,activePattern:t})})]},o)})]})]})}export{_ as B,he as P,fe as a,N as c};
